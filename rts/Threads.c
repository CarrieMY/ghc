...
checkBlockingQueues (Capability *cap, StgTSO *tso)
{
    ...
    for (bq = tso->bq; bq != (StgBlockingQueue*)END_TSO_QUEUE; bq = next) {
        ...
        // We need to always ensure we untag bh.  While it might seem a senible
        // assumption that bh will never be tagged, the GC could shortcut the
        // indirection and put a tagged pointer into the indirection.
        //
        ...
        // a garbage pinfo, which very, very unlikely would have been equal to
        // stg_BLACKHOLE_info.  Thus while the code would have done the wrong
        // thing the result would be the same in almost all cases.
