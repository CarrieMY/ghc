...
Subtle points:
* We do eta-reduce (\x. f <expensive> x).  In the main eta-reducer we
  do not eeat-reduce this becuase doing so might reduce the arity of
  the expression (from 1 to zero).  But for rule-matching we /do/
  want to match template (f a) against target (\x. f <expensive> x),
  with a := <expensive>
